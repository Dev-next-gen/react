...
describe('ReactFlightTurbopackLedgers', () => {
  ...
  // A set ledger nets entries by value, and a Set can only compare an object
  // by reference, so a non-primitive entry — a ledger total included — is
  ...
  // A symbol that is not registered has no name to net it by, so it is
  // refused at the write like a non-primitive; a registered symbol
  // round-trips (see the kinds test).
  // @gate enableFlightLedgers
  ...
  // Ledger totals come back in the order they were declared — declare
  // `[A, B]`, destructure `[a, b]` — and each resolves on its own. One
  // nothing writes to resolves to its kind's empty total.
  // @gate enableFlightLedgers
  ...
  it('round-trips every kind', async () => {
    ...
    // A set promises membership, never iteration order, so each entry is
    // found by predicate. `-0` and `0` are one entry.
  ...
  // A nested capture of the same ledger keeps its subtree's writes; the
  // enclosing capture nets only what was written outside the nested one.
  // @gate enableFlightLedgers
