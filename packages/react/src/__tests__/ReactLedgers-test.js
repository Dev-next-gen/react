...
describe('ReactLedgers', () => {
  beforeEach(() => {
    jest.resetModules();
    // The ledger API is exported from the experimental server entry point.
  ...
  it('creates opaque, frozen ledger types', () => {
    ...
    for (let i = 0; i < types.length; i++) {
      if (__DEV__) {
        // Freezing is a development-only guard, like the one on elements.
  ...
  // A valid write outside any Flight request is dropped: there is nothing to
  // take delivery of it, and nothing to report an error through either.
  // @gate enableFlightLedgers
