...
function markUnitDirty(ledgers: RequestLedgers, unit: Unit): void {
  if (unit.dirty) {
    return;
  }
  unit.dirty = true;
...
function recordUnitReference(unit: null | Unit, row: number): void {
  ...
  // Avoid allocating a Set when only one row is reused between flushes.
...
export function addToLedgerImpl(type: Ledger<empty>, entry: mixed): void {
  const unit = resolveRunningUnit();
  if (unit === null) {
  ...
  let cell = dirty.get(type);
  if (cell === undefined) {
    dirty.set(type, (cell = createLedgerCell(type)));
  }
...
export function captureLedgersImpl<T, V: $ReadOnlyArray<Ledger<empty>>>(
  input: T,
  ledgers: V,
  ...
  for (let i = 0; i < ledgers.length; i++) {
    ...
    totals.push({
      ...
      request,
      type: ledger,
      id: declareLedgerTotal(request, requestLedgers, ledger),
      then() {
...
// The client needs the creator chain even when ancestors have no writes. It
// walks this chain to find the nearest capture for each ledger and determine
// which writes belong to it. Declare ancestors first so it can connect each
// unit as it arrives.
